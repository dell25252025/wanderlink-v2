...
import type { DocumentData, Timestamp, PermissionStatus } from 'firebase/firestore';
...
export default function ChatClientPage({ otherUserId }: { otherUserId: string }) {
  ...
  const requestPermission = async (permission: CameraPermissionType): Promise<boolean> => {
    let status: PermissionStatus;
    try {
      const result = await Camera.checkPermissions();
      status = result[permission];

      if (status.state === 'prompt' || status.state === 'prompt-with-rationale') {
        const newResult = await Camera.requestPermissions({ permissions: [permission] });
        status = newResult[permission];
      }
      ...
      if (status.state === 'denied') {
        toast({
          ...
          description: `Pour utiliser cette fonctionnalité, veuillez activer la permission dans les paramètres de l\'application.`,
        });
        return false;
      }
  ...
  const handleDownloadImage = useCallback(async () => {
    ...
    try {
        const hasPermission = await requestPermission('photos'); // Using 'photos' which covers public storage
  ...
  const takePicture = useCallback(async (source: CameraSource) => {
    const hasPermission = await requestPermission(source === CameraSource.Camera ? 'camera' : 'photos');
  ...
  const handleStartCall = useCallback(async (isVideo: boolean) => {
    const permissionsToRequest: CameraPermissionType[] = isVideo ? ['camera', 'microphone'] : ['microphone'];
