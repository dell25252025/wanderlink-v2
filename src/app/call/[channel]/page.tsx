...
export default function CallPage() {
  ...
  useEffect(() => {
      ...
      try {
        client.on('user-published', async (user, mediaType) => {
          ...
          if (mediaType === 'video' && user.videoTrack) {
            // Need a small delay to ensure the DOM element is ready if it's being rendered conditionally
            setTimeout(() => {
        ...
        client.on('user-unpublished', user => {
            // Do not remove user immediately, wait for 'user-left'
             if (user.videoTrack) {
        ...
        // Generate Token
        const tokenResult = await generateAgoraToken(channelName, 0); // Using 0 for UID as we are using string UIDs for auth but integer UIDs for Agora are simpler for now, or match them if needed. 
        // Note: The generateAgoraToken in firebase-actions uses 0 if uid is a string.
        // Ideally, we should use the same UID logic.
        // Let's rely on the server action handling the UID.
        ...
        if (tokenResult.success && tokenResult.token) {
            token = tokenResult.token;
             ...
             // Fallback for testing without token if app certificate is not enforcing it (usually it does)
             // or handle error
        }

        // For this implementation, let's assume we use the integer UID 0 (or generated by Agora) 
        // If we pass a string UID to join, ensure the token was generated for that string UID (or 0 if using wildcard).
        // The current generateAgoraToken uses 0. So we should join with 0 or null to let Agora assign.
        // BUT, we want to identify users.
        
        // Correct approach with current generateAgoraToken logic:
        // It generates a token for UID 0. So we must join with UID 0 (or null).
    ...
    return () => {
        // Cleanup function
         // We can't act here easily because leaveCall is async and this is sync cleanup
         // But we should ensure tracks are stopped.
        ...
        const unsubscribe = onSnapshot(callDocRef, (doc) => {
            if (doc.exists()) {
                ...
                if (callData.status === 'ended' || callData.status === 'rejected') {
                     // We should only leave if the status changed to ended/rejected remotely
                     // For now, simple check.
                     if(!isJoining) { // avoid leaving while joining
                         leaveCall();
                     }
                }
            }
  ...
  const leaveCall = async () => {
    ...
    if(channelName) {
        const callDocRef = doc(db, 'calls', channelName);
        // Only update to ended if it's not already ended/rejected to avoid loops or overwrites
         // Best effort.
